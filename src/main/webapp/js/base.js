...
google.devrel.samples.hello.CLIENT_ID =
    '474246175047-a89rie7k88jgsda6npcsdutdgmr6seih.apps.googleusercontent.com';
/**
...
google.devrel.samples.hello.listAccounts = function(){
  gapi.client.helloworld.greetings.listAccounts().execute(
      function(resp) {
        if (!resp.code) {
          resp.items = resp.items || [];
          for (var i = 0; i < resp.items.length; i++) {
            var element = document.createElement('div');
            element.classList.add('row');
            element.innerHTML = resp.items[i].name;
            document.getElementById('outputLog').appendChild( element );
          }
        }
      });
...
google.devrel.samples.hello.enableButtons = function() {
  ...
  document.getElementById('listAccounts').onclick = function() {
    google.devrel.samples.hello.listAccounts();
  };


  console.log('enabling buttons gapi.client state:');
  console.log(gapi.client);
